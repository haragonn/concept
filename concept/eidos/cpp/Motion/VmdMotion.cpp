...
bool VmdMotion::LoadVmdMotionFromFile(const char* pFileName, PmxModel& mdl, bool bLoop)
{
	pPmx_ = &mdl;
	bLoop_ = bLoop;
	elapsedTime_ = 0.0f;
...
void VmdMotion::UpdatePmd(float speed)
{
	...
	fill(pPmd_->vecBoneMatrix_.begin(), pPmd_->vecBoneMatrix_.end(), Matrix4x4::Identity());

	for(auto& md : umapMotionData_){
		auto itBone = pPmd_->mapBone_.find(md.first);
		if(itBone == pPmd_->mapBone_.end()){ continue; }
		...
		for(int i = 0; i < 4; ++i){
			for(int j = 0; j < 4; ++j){
				pPmd_->vecBoneMatrix_[node.index].r[i][j] = mat.r[i].m128_f32[j];
			}
		}
	}

	RecursiveMatrixMultiply(&pPmd_->mapBone_["センター"], Matrix4x4::Identity());
}

void VmdMotion::UpdatePmx(float speed)
{
	if(!pPmx_){ return; }
	...
	fill(pPmx_->vecBoneMatrix_.begin(), pPmx_->vecBoneMatrix_.end(), Matrix4x4::Identity());

	for(auto& md : umapMotionData_){
		auto itBone = pPmx_->mapBone_.find(md.first);
		if(itBone == pPmx_->mapBone_.end()){ continue; }
		auto node = itBone->second;
		if(node.index == -1){ continue; }
		...
		for(int i = 0; i < 4; ++i){
			for(int j = 0; j < 4; ++j){
				pPmx_->vecBoneMatrix_[node.index].r[i][j] = mat.r[i].m128_f32[j];
			}
		}
	}

	XRecursiveMatrixMultiply(&pPmx_->mapBone_["センター"], Matrix4x4::Identity());
}
...
void VmdMotion::RecursiveMatrixMultiply(VmdBone * node, const Matrix4x4 & mat)
{
	pPmd_->vecBoneMatrix_[node->index] *= mat;

	for(auto& cnode : node->vecChildrenPtr){
		RecursiveMatrixMultiply(cnode, pPmd_->vecBoneMatrix_[node->index]);
	}
}

void VmdMotion::XRecursiveMatrixMultiply(VmdBone* node, const Matrix4x4& mat)
{
	pPmx_->vecBoneMatrix_[node->index] *= mat;

	for(auto& cnode : node->vecChildrenPtr){
		XRecursiveMatrixMultiply(cnode, pPmx_->vecBoneMatrix_[node->index]);
	}
}
